...
<link rel="stylesheet" type="text/css" href="<%=request.getContextPath()%>/css/admin.css"/>
    ...
    <section>
        ...
        <div class="texts">
        <div id="listmenu">
        <p onclick="location.assign('<%=request.getContextPath()%>/mypage/mypageWriting.do?id=<%=logInMember.getMemberId()%>');">전체글</p>
        <p style="background-color: lavender;" onclick="location.assign('<%=request.getContextPath()%>/mypage/mypageWroteQna.do?id=<%=logInMember.getMemberId()%>');">질문 글</p>
            ...
            <div>
                <table id="list">
					...
					<%if(qlist.isEmpty()) {%>
						<tr>
							<td>Q&A</td>
							<td colspan="4">아직 작성한 질문글이 없습니다:(</td>
						</tr>
					<%}else {
						for(Qna q : qlist) {%>
	                    <tr>
	                    	<td>Q&A</td>
	                        <td onclick="location.assign('<%=request.getContextPath()%>/qna/qnaView.do?qnaNo=<%=q.getQnaNo()%>');">
	                        	<%=q.getReviewTitle() %>
	                        </td>
